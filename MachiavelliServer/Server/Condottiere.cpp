...
void Condottiere::PerformCharacteristic(std::shared_ptr<GameManager> manager, std::shared_ptr<Player> player)
{
	...
	//while (!player_chosen)
	//{
	//	std::string input = networkServices->PromptClient(socket);
	...
	//			{
	//				name_destroyed_building = build_cards.at(number)->GetName();
	//				player->RemoveGold(build_cards.at(number)->GetCost() - 1);
	//				output = "You have destroyed building: " + name_destroyed_building + ", it cost you "
	//					+ std::to_string(build_cards.at(number)->GetCost() - 1)
	//					+ ", You got " + std::to_string(player->GetGold()) + " gold left.\n";
